...
class explorer(pd.DataFrame):
    ...
    def frequency_table(self,feature):
        ...
        feature (str): The feature for which to generate the frequency table.
        
        Returns:
        pd.DataFrame: A DataFrame containing the frequency table.
        """
        if feature not in self.columns:
            raise ValueError(f"Feature '{feature}' not found in DataFrame.")
    
        # Count the frequencies of each unique value in the feature
        frequency_table = self[feature].value_counts().reset_index()
        
        # Rename the columns for clarity
        frequency_table.columns = [feature, 'Frequency']
        
        # Sort the frequency table if the feature is not categorical
        if isinstance(self[feature].dtype, pd.CategoricalDtype) or self[feature].dtype != object:
            frequency_table = frequency_table.sort_values(by=feature, ascending=True)
