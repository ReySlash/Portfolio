...
try:
    del pd.DataFrame.params_optimizer
except AttributeError:
    pass

@pd.api.extensions.register_dataframe_accessor("params_optimizer")
class params_optimizer(pd.DataFrame):


    def plot_explained_variance(self):
        pca = PCA(random_state=42)
        X_reduced = pca.fit_transform(self)
        cumulative_exp_var = pca.explained_variance_ratio_.cumsum()*100
        sns.lineplot(
            x=list(range(1,self.shape[1]+1)),
            y=cumulative_exp_var,
            marker='o',
            color="r"
        )
        plt.yticks(list(range(5,101,5)))
        plt.xticks(list(range(1,self.shape[1]+1)))
        plt.xlabel("Number of Components")
        plt.ylabel("Cumulative Explained Variance")
        plt.grid()
        
    def pca_kmeans_elbow_study(self,pca_n_components_range, n_clusters_range):
    ...
    def pca_kmeans_study(self, pca_n_components_range = None, n_clusters_range=0):
        results = []
        if pca_n_components_range is None:
            ...
            for n_clusters in list(range(2, n_clusters_range)):
                ...
                kmeans.fit(self)                                     # Only fit the KMeans model
                
                inertia = kmeans.inertia_                                # Get inertia for the current clustering
                silhouette_avg = silhouette_score(self, kmeans.labels_)  # Compute silhouette score
            ...
            for n_components in list(range(2, pca_n_components_range)):
                pca = PCA(n_components=n_components, random_state=42)  # Initialize PCA with the current number of components
                data_pca = pca.fit_transform(self)                       # Apply PCA transformation on data
    ...
    def hierarchical_dendogram_study(self,pca_n_components_range): 
        ...
        for n_components in pca_n_components_range:
            ...
            pca_result = pca.fit_transform(self)
    ...
    def hcl_study(self,pca_n_components_range, n_clusters_range):
    ...
    def knee_study(self, max_n_component):
    ...
    def dbscan_study(self,eps_values,min_samples,pca_n_components_range=None):
        # Apply PCA
        ...
        if pca_n_components_range:
            ...
            for eps, min_s in dbscan_params:
                ...
                # Calculate silhouette score, ignoring noise points
                # if n_clusters_ > 1:  # Silhouette score is not defined for a single cluster
                #     sil_score = silhouette_score(self[labels != -1], labels[labels != -1])
                # else:
                #     sil_score = None  # Use None for clarity

                valid_mask = labels != -1
                if valid_mask.sum() > 0:  # Asegurarse de que no todos sean ruido
                    sil_score = silhouette_score(self[valid_mask], labels[valid_mask])
                else:
                    sil_score = None
