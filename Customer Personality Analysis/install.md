# Customer Personality Analysis guide installation
...
activate customer_personality_analysis
```
...
To move beyond notebook prototyping, all reusable code should go into the `customer_personality_analysis/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `customer_personality_analysis` folder and use the modules inside your notebooks :
...
```python
from customer_personality_analysis.utils.paths import data_dir
data_dir()
```
